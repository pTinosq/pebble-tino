...
async function getTime(args: Record<string, unknown>): Promise<string> {
  const place = typeof args.location === "string" ? args.location.trim() : "";

  // No place given → server local time.
  let timeZone = Intl.DateTimeFormat().resolvedOptions().timeZone;
  let label = "local time";

  if (place) {
    // Resolve the place name to an IANA timezone via the geocoding API.
    // A country entity (e.g. "China") has no timezone since it spans several, so
    // among the hits that DO have a timezone we pick the most populous — that
    // avoids grabbing a tiny same-named village in another country.
    ...
    const geo = (await geoRes.json()) as {
      results?: Array<{ name: string; country?: string; timezone?: string; population?: number }>;
    };
    const withTz = (geo.results ?? []).filter((r) => r.timezone);
    withTz.sort((a, b) => (b.population ?? 0) - (a.population ?? 0));
    const loc = withTz[0];
    if (!loc?.timezone) {
